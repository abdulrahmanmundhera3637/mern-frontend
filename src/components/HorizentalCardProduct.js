      ...
      <div className='relative'>
        <div className='flex items-center gap-4 md:gap-6 overflow-scroll scrollbar-none transition-all' ref={scrollElement}>
          {loading ? (
            ...
            data.map((product) => (
              <Link key={product._id} to={`/product/${product._id}`} className='w-full min-w-[280px] md:min-w-[320px] max-w-[180px] md:max-w-[320px] h-36 bg-white rounded-sm shadow flex'>
                ...
                <div className='p-2 grid'>
                  ...
                  <div className='flex gap-1'>
                  <p className='text-slate-500 line-through mb-1'>{displayPKRCurrency(product.price)}</p>
                  </div>
                  <button className='text-sm bg-red-600 hover:bg-red-700 text-white  rounded-full' onClick={(e) => handleAddToCart(e, product._id)}>Add to Cart</button>
                </div>
              </Link>
            ))
          )}
